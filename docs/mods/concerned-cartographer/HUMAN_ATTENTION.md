...
- Status: PARTIALLY RESOLVED 2026-08-28 — owner created the Sentry project and provided the DSN; it is embedded in `CrashReportingConfig.EmbeddedSentryDsn` and ingestion was verified live (envelope POST → HTTP 200, event 9e3d83be…). STILL OWNER-ONLY before publish: in the Sentry project settings, turn ON "Prevent Storing of IP Addresses" and the Data Scrubbers (PRIVACY.md promises both), add the sensitive-field list, and configure the four alerts (new issue / regression / high-frequency / persistence-tag) per CRASH_REPORTING.md. The pasted onboarding snippet's `AutoSessionTracking` was deliberately NOT adopted (session telemetry is beyond the consented policy). 2026-09-03 addendum: the pipeline is field-proven end-to-end — the owner's 0.9.0 smoke produced a real captured event (CONCERNED-CARTOGRAPHER-2, event #109, the pin-update NullReferenceException fixed in RC14), confirming consent gating, capture, scrubbing, and ingestion work live; the Sentry-side scrubbing settings + four alerts remain the outstanding owner-only work.
