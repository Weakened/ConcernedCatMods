...
/// <summary>Carried across from Concerned Cartographer's own suite verbatim,
/// with WorldPoint renamed to NpcPoint and nothing else changed. It is here
/// because the behaviour moved here, and it is unchanged because the whole claim
/// being made about the move is that the behaviour did not change: every
/// threshold, the doubling pause, the capacity, the geometry, and the in-game
/// case from b35c2d8 that the memory was written for.
///
/// <b>What it is about.</b> What he learns from a walk that did not work
/// (#310): the spot, and
...
public sealed class NpcWalkSetbacksTests
{
    // Stopped at the origin, floor at y = 10, walking north (+Z) into something
    // just ahead.
    private static readonly NpcPoint Stopped = At(0f, 0f);
    private static readonly NpcPoint North = At(0f, 1.5f);

    private static NpcPoint At(float x, float z, float y = 10f)
    {
        return new NpcPoint(x, y, z);
    }

    private static NpcWalkSetbacks StoppedGoingNorth(float now = 0f)
    {
    ...
    public void TheWallFromTheSignOffSessionStopsTheSecondSpotBehindItToo()
    {
        ...
        var setbacks = new NpcWalkSetbacks();
        NpcWalkSetback first = setbacks.Remember(
            new NpcPoint(-34.5f, 81.9f, 29.8f),
            new NpcPoint(-37.0f, 81.3f, 23.0f),
            new NpcPoint(-36.9f, 81.4f, 24.5f),
            now: 100f);
        Assert.True(first.HasPlace);

        NpcPoint[] toTheOtherSpot =
        {
            new NpcPoint(-30.0f, 83.0f, 18.0f),
            new NpcPoint(-37.2f, 81.3f, 21.5f),
            new NpcPoint(-36.9f, 81.4f, 24.5f),
            new NpcPoint(-35.0f, 82.0f, 29.0f),
        };
        ...
        NpcPoint[] roundTheOtherSide =
        {
            new NpcPoint(-37.0f, 81.3f, 23.0f),
            new NpcPoint(-40.0f, 81.3f, 20.0f),
            new NpcPoint(-42.0f, 83.5f, 30.0f),
        };
    ...
    {
        NpcWalkSetbacks setbacks = StoppedGoingNorth();
    ...
    {
        NpcWalkSetbacks setbacks = StoppedGoingNorth(now: 10f);
        NpcPoint[] through = { At(0f, -6f), At(0f, 6f) };
    ...
    public void TheSameTroubleAgainIsLeftAloneLongerUpToFiveMinutes()
    {
        ...
        {
            NpcWalkSetback setback = setbacks.Remember(At(attempt * 4f, 8f), Stopped, North, now);
    ...
    public void AnotherPlaceOrAnotherWayIsAFirstTime()
    {
        ...
        NpcWalkSetback elsewhere = setbacks.Remember(At(20f, 8f), At(20f, 0f), At(20f, 1.5f), now: 1f);
        NpcWalkSetback otherWay = setbacks.Remember(At(0f, -8f), Stopped, At(0f, -1.5f), now: 2f);
    ...
    {
        NpcWalkSetbacks setbacks = StoppedGoingNorth(now: 0f);

        NpcWalkSetback muchLater = setbacks.Remember(
            At(0f, 8f), Stopped, North, now: 60f + NpcWalkSetbacks.RecallSeconds + 1f);

        Assert.Equal(1, muchLater.Strikes);
        Assert.Equal(NpcWalkSetbacks.FirstPauseSeconds, muchLater.PauseSeconds);
    }
    ...
    {
        var setbacks = new NpcWalkSetbacks();
        NpcWalkSetback setback = setbacks.Remember(At(0f, 8f), now: 0f);
    ...
    public void WithNoWayToTellWhichWayHeWasGoingOnlyTheSpotIsLeftAlone()
    {
        ...
        NpcWalkSetback setback = setbacks.Remember(Stopped, Stopped, Stopped, now: 0f);
        ...
        NpcWalkSetback towardsTheSpot = setbacks.Remember(At(0f, 8f), Stopped, Stopped, now: 2f);
    ...
    {
        var setbacks = new NpcWalkSetbacks();
        for (int index = 0; index < NpcWalkSetbacks.Capacity + 3; index++)
        {
            setbacks.Remember(At(index * 10f, 8f), now: index);
        }

        Assert.Equal(NpcWalkSetbacks.Capacity, setbacks.Remembered.Count);
        Assert.False(setbacks.RefusesSpot(At(0f, 8f), now: 20f));
        Assert.True(setbacks.RefusesSpot(At((NpcWalkSetbacks.Capacity + 2) * 10f, 8f), now: 20f));
        Assert.Equal(
            Enumerable.Range(3, NpcWalkSetbacks.Capacity).Select(index => index * 10f),
    ...
    public void ARouteOfFewerThanTwoCornersGoesNowhere()
    {
        ...
        Assert.False(setbacks.RefusesRoute(new NpcPoint[0], now: 1f));
    ...
    {
        Assert.Equal(seconds, NpcWalkSetbacks.PauseFor(strikes));
    }
}
