...
- **Per-world trip recording (CT-016).** Pulled-cart trips (position, grade, speed, load) are recorded as bounded sample sequences and persisted to Teamster's own sidecar file — never a Valheim save. Atomic temp-file writes survive a kill at any moment; a versioned header carries the owning world's UID, so world A's trips can never load into world B (filename and header both); malformed rows are skipped and reported, foreign or future files are backed up untouched instead of overwritten. Trip count is capped with oldest-trip pruning and a visible retention setting.
