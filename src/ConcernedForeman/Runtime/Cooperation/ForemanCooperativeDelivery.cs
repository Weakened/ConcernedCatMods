...
internal sealed class ForemanCooperativeDelivery : ICooperativeDelivery
{
    ...
    private sealed class RuntimeCustody : ICooperationCustody
    {
        ...
        /// <summary><c>order-t-revision</c>, shortened to a slug: the custody
        /// record's own transfer id scheme (agent D's <c>CustodyIds.ForTransfer</c>,
        /// which replaces this once both land), unique because every persisted
        /// transfer moves the record's revision.</summary>
        public RequestId NextTransferId(OrderId order)
        {
            string suffix = "-t-" + _runtime.View.Revision.ToString(CultureInfo.InvariantCulture);
